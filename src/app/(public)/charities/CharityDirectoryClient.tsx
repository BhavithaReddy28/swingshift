...
export function CharityDirectoryClient({ initialCharities }: { initialCharities: Charity[] }) {
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="glass-panel p-6 rounded-2xl border border-white/10 flex flex-col md:flex-row gap-4 items-center justify-between">
        {/* Search input */}
        <div className="relative w-full md:w-96">
          ...
          <input
            ...
            className="w-full bg-black/40 border border-white/10 rounded-xl pl-11 pr-4 py-3 text-sm text-white placeholder-gray-500 focus:outline-none focus:border-orange-500 transition-colors"
          />
        </div>
      ...
      {filteredCharities.length === 0 ? (
        <div className="glass-panel p-12 text-center rounded-2xl border border-white/10">
          ...
          <p className="text-gray-400 text-sm mt-1">Try adjusting your search terms or filter selection.</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredCharities.map((charity) => (
            <div
              key={charity.id}
              className="glass-panel rounded-2xl border border-white/10 overflow-hidden flex flex-col hover:border-orange-500/40 transition-all duration-300 group"
            >
              {/* Hero Image */}
              ...
              <div className="p-6 flex-1 flex flex-col justify-between space-y-4">
                ...
                <Link
                  ...
                  className="w-full py-2.5 rounded-xl bg-white/5 hover:bg-orange-500/20 text-orange-400 font-semibold text-sm transition-all border border-orange-500/30 flex items-center justify-center gap-2 group-hover:bg-orange-500 group-hover:text-white"
                >
                  View Profile & Donate
