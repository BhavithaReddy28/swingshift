      ...
      <div className="glass-panel p-6 sm:p-8 rounded-3xl border border-border space-y-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-muted">Select period month (first of month) and draw mode.</p>
          </div>
        </div>
        ...
        <form onSubmit={handleCreateDraw} className="grid grid-cols-1 sm:grid-cols-3 gap-4 items-end">
          <div>
            ...
            <input
              ...
              className="w-full bg-input border border-border rounded-xl px-4 py-3 text-sm text-white focus:outline-none focus:border-amber-500"
              required
            />
          </div>
          ...
          <div>
            ...
            <select
              ...
              className="w-full bg-input border border-border rounded-xl px-4 py-3 text-sm text-white focus:outline-none focus:border-amber-500"
            >
              <option value="random" className="bg-ink-elevated text-white">
                Random Mode (Crypto-Random Seeded)
              </option>
              <option value="algorithmic" className="bg-ink-elevated text-white">
          ...
          <button
            ...
            className="py-3 rounded-xl bg-gradient-to-r from-primary to-primary-glow text-white font-semibold text-sm hover:from-primary-glow hover:to-primary transition-all shadow-lg flex items-center justify-center gap-2"
          >
            ...
            >
              <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 border-b border-border pb-4">
                <div>
                  ...
                  <span className="text-xs text-muted">Mode: <strong className="text-white uppercase">{draw.mode}</strong></span>
                </div>
                  ...
                  {!isPublished ? (
                    <>
                      ...
                      <button
                        ...
                        className="px-5 py-2 rounded-xl bg-gradient-to-r from-[#FF5A36] to-primary-glow text-white shadow-lg text-xs font-bold flex items-center gap-1.5 hover:from-[#FF5A36]/90 hover:to-primary transition-all"
                      >
                        {publishLoading ? (
                          <Loader2 className="w-4 h-4 animate-spin" />
                        ) : (
                    ...
                    <div className="flex items-center gap-2 text-xs text-support font-bold bg-teal-500/10 px-3.5 py-1.5 rounded-full border border-teal-500/20">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div>
                  <span className="text-xs text-muted block mb-2 font-semibold uppercase tracking-wider">
                    Winning Numbers
                  </span>
                  <div className="flex gap-2">
                    {draw.winning_numbers && draw.winning_numbers.length === 5 ? (
                      ...
                      <span className="text-xs text-muted italic">Not generated yet (Run Simulate)</span>
                    )}
                  </div>
                </div>

                <div>
                  <span className="text-xs text-muted block mb-1 font-semibold uppercase tracking-wider">
                    Total Pool
                  </span>
                ...
                <div>
                  ...
                  <div className="text-xs text-gray-300">
                    In: <strong className="text-support">{formatCurrency(draw.rollover_in_pence)}</strong> | Out: <strong className="text-amber-400">{formatCurrency(draw.rollover_out_pence)}</strong>
                  </div>
                </div>
              </div>
              ...
              {draw.mode === "algorithmic" && draw.frequency_snapshot && (
                <div className="p-4 rounded-2xl bg-input border border-border space-y-2">
                  ...
                  <div className="flex flex-wrap gap-2 text-xs">
                    {Object.entries(draw.frequency_snapshot)
                      .slice(0, 10)
                      .map(([score, count]) => (
                        <span key={score} className="px-2 py-1 rounded bg-ink-elevated text-gray-300">
              ...
              {isSimulating && simulationResult && (
                <div className="p-6 rounded-2xl bg-amber-500/10 border border-amber-500/30 space-y-4">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 text-xs">
                    <div>
                      <span className="text-muted block">Eligible Entries:</span>
                      <strong className="text-white text-base">{simulationResult.eligibleCount}</strong>
                    </div>
                    <div>
                      <span className="text-muted block">Jackpot Winners (5 Match):</span>
                      <strong className="text-amber-400 text-base">{simulationResult.prizeSplit.winnerCounts.tier5}</strong>
                    </div>
                    <div>
                      <span className="text-muted block">Estimated Rollover Out:</span>
                      <strong className="text-support text-base">{formatCurrency(simulationResult.prizeSplit.rolloverOutPence)}</strong>
                    </div>
                  </div>
                </div>
              )}
